...
namespace jp.kshoji.unity.vst3nativehost.mcp.runtime
{
    public partial class Tool_VstHost
    {
        ...
        [Description(
            "Send Note On to a loaded plugin. Play Mode / running build required for audible result. " +
            "Channel 0–15.")]
        ...
        )
        {
            ...
            {
                if (!TryRequireAudioSession("vst3-note-on", out var playErr))
                    return playErr!;
        ...
        )
        {
            return MainThread.Instance.Run(() =>
            {
                if (!TryRequireAudioSession("vst3-note-off-all", out var playErr))
                    return playErr!;
        ...
        [Description("Send Note Off. Play Mode / running build required.")]
        ...
        )
        {
            return MainThread.Instance.Run(() =>
            {
                if (!TryRequireAudioSession("vst3-note-off", out var playErr))
                    return playErr!;
        ...
        )
        {
            return MainThread.Instance.Run(() =>
            {
                if (!TryRequireAudioSession("vst3-send-cc", out var playErr))
                    return playErr!;
        ...
        [Description(
            "Send MIDI Program Change (not host SetProgram). For host programs use vst3-set-program.")]
        ...
        )
        {
            return MainThread.Instance.Run(() =>
            {
                if (!TryRequireAudioSession("vst3-send-pc", out var playErr))
                    return playErr!;
        ...
        [Description("Send Pitch Bend (14-bit amount, center 8192).")]
        ...
        )
        {
            return MainThread.Instance.Run(() =>
            {
                if (!TryRequireAudioSession("vst3-send-pitch", out var playErr))
                    return playErr!;
        ...
        )
        {
            return MainThread.Instance.Run(() =>
            {
                if (!TryRequireAudioSession("vst3-send-aftertouch", out var playErr))
                    return playErr!;
        ...
        [Description("Send a raw MIDI 1.0 short message (status/data1/data2).")]
        ...
        )
        {
            return MainThread.Instance.Run(() =>
            {
                if (!TryRequireAudioSession("vst3-send-midi1", out var playErr))
                    return playErr!;
