...
namespace jp.kshoji.unity.vst3nativehost.mcp
{
    public partial class Tool_VstHost
    {
        [AiTool(
            ...
            ReadOnlyHint = true)]
        [Description(
            "Get opaque plugin state blob as Base64 (VHS1 + component/controller). " +
            "Large blobs may truncate in the text response unless maxBase64Chars is raised.")]
        ...
        [Description(
            "Apply opaque plugin state from Base64 (from vst3-state-get or an external blob). " +
            "Prefer vst3-preset-apply for project VstPresetAsset files.")]
