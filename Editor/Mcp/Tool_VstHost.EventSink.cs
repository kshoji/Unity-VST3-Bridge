...
namespace jp.kshoji.unity.vst3nativehost.mcp
{
    ...
    {
        internal const string DefaultMidiObjectName = "__VstHostMcpMidi";
        ...
        )
        {
            return MainThread.Instance.Run(() =>
            {
                ...
                // channel / defaultVelocity are serialized fields — set via SerializedObject
                var so = new UnityEditor.SerializedObject(sink);
                so.FindProperty("channel").intValue = Mathf.Clamp(channel, 0, 15);
                so.FindProperty("defaultVelocity").intValue = Mathf.Clamp(defaultVelocity, 1, 127);
