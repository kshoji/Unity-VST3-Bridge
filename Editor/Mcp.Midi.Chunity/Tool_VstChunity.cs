...
namespace jp.kshoji.unity.vst3nativehost.mcp.midi
{
    ...
    {
        const string DefaultObjectName = "__VstHostMcpChuck";
        ...
        )
        {
            return MainThread.Instance.Run(() =>
            {
                ...
                so.FindProperty("target").enumValueIndex = (int)kind;
                so.FindProperty("outputGain").floatValue = outputGain;
        ...
        )
        {
            return MainThread.Instance.Run(() =>
            {
                ...
                if (!string.IsNullOrWhiteSpace(virtualDeviceIdOverride))
                    so.FindProperty("virtualDeviceIdOverride").stringValue = virtualDeviceIdOverride.Trim();
                so.ApplyModifiedPropertiesWithoutUndo();
                link.Apply();
