...
namespace jp.kshoji.unity.vst3nativehost.mcp.midi
{
    ...
    public partial class Tool_VstMidi
    {
        ...
        [Description(
            ...
            "Creates __VstHostMcpMidi when gameObjectName is empty. Prefer Play Mode + MidiManager.")]
        ...
        )
        {
            return MainThread.Instance.Run(() =>
            {
                ...
                so.FindProperty("channelFilter").objectReferenceValue = filter;
                so.FindProperty("adapter").objectReferenceValue = go.GetComponent<VstHostMidiAdapter>();
                so.FindProperty("unregisterAdapterFromMidiManager").boolValue =
                    unregisterAdapterFromMidiManager;
        ...
        )
        {
            return MainThread.Instance.Run(() =>
            {
                ...
                so.FindProperty("adapter").objectReferenceValue = go.GetComponent<VstHostMidiAdapter>();
                so.FindProperty("graph").objectReferenceValue = go.GetComponent<VstAudioGraph>();
